				...
				<section id="content">
					<div class="back_icon">
						<a onclick="history.back()"><img
							src="./sources/icons/arrow_back.svg" alt="arrow_back"></a>
					</div>
					<div id="postview_Wrapper">
						<div class="title">
							<p>${note.title}</p>
							<c:if
								test="${sessionScope.userInfo != null && sessionScope.userInfo.ac_idx == note.upac_idx}">
								<div>
									<button>
										<a href="noteedit.do?noteidx=${note.note_idx}">edit</a>
									</button>
									<button>
										<a href="notedelete.do?noteidx=${note.note_idx}">delete</a>
									</button>
								</div>
							</c:if>
						</div>
						<div class="writer_info">
							<div class="writer">
								<img
									src="${(note.img == null or empty note.img) ? './sources/icons/profile.svg' : note.img}"
									alt="writer_profile"> <a
									href="userPage.do?acIdx=${note.ac_idx}">${note.nickname}</a> 
								<c:if test="${user.ac_idx != note.ac_idx}">
									<form id="followForm"
										style="display: inline; margin: 0; padding: 0;">
										<button id="followBtn" type="submit"
											data-writer-idx="${note.ac_idx}" data-nidx="${note.note_idx}">
							...
							<div class="like_share">
								<div>
									<p>
										<span>view : </span><span>${note.view_count}</span>
									</p>
								</div>
								<form id="likeForm"
									style="display: inline; margin: 0; padding: 0;">
									<button id="likeBtn" type="submit"
										...
										<img id="likeImg"
											src="${contextPath}${isLiking ? '/sources/icons/fill_heart.png' : '/sources/icons/heart.svg'}"
											alt="heart"> <span id="likeCount">${note.like_num}</span>
									</button>
								</form>
							</div>
						</div>

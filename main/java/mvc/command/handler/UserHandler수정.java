...
public class UserHandler수정 implements CommandHandler {
    ...
    private String processLogout(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        // 자동 로그인 쿠키 삭제
        Cookie autoLoginCookie = new Cookie("autoLoginUserEmail", "");
        autoLoginCookie.setPath("/");
        autoLoginCookie.setMaxAge(0);
        response.addCookie(autoLoginCookie);

        if (session != null) {
            session.invalidate(); // 세션 무효화 -> Listener가 호출되어 loginUsers 맵에서 제거
        }

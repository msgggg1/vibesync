		...
		if (!"POST".equalsIgnoreCase(requestMethod)) { // 요청방식이 POST가 아닌 경우 (GET 방식)
			if(session.getAttribute("userInfo") != null) { // 로그인 되어 있는 상태
				...
				if (session.getAttribute("referer") == null) {
                    // HTTP Referer 헤더에서 이전 페이지 주소를 가져옴
                    String httpReferer = request.getHeader("Referer");
                    
                    // 이전 페이지가 존재하고, 그 페이지가 로그인 페이지 자신이 아닐 경우에만 세션에 저장
			...
			if (accessType.equals("login")) { // 로그인
				...
				if (userInfo != null) { // 로그인 성공
					...
					// 2. 세션에서 '이전 페이지 주소(referer)'가 있는지 확인
					String referer = (String) session.getAttribute("referer");
					if (referer != null && !referer.isEmpty()) {
						// 3-1. 있다면, 사용했던 referer 속성을 세션에서 삭제하고 그 주소로 이동
						session.removeAttribute("referer");
						response.sendRedirect(referer);
						} else {// 3-2. 없다면, 기본 메인 페이지로 이동
